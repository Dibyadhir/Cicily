        ...
        if uname and pwd:
            ...
            for i in dsig:
                ...
                if dsig =='superadmin':
                    ...
                    logo=clientdetails.objects.filter(clientname=username)
                    x=user_report.objects.filter(clientname=username)
                    y=user_report.objects.filter(clientname=username)
                    campaign_names= user_report.objects.filter(clientname=username).values_list('campaign_name')
                    templist=[]
                    ...
                    context={'x':x,'y':y,'campaigns':campaigns,'campaign_names':templist,'logo':logo}
...
def campaign_details(request):
    if request.method == 'POST':
        ...
        campaign_names = user_report.objects.filter(clientname=username).values_list('campaign_name', flat=True)
        campaigns=user_report.objects.all()
        context = {'campaign': campaign,'campaigns':campaigns,'campaign_names':campaign_names}
        return render(request, 'campaign.html', context)
    else:
        campaign_names = user_report.objects.filter(clientname=username).values_list('campaign_name', flat=True)
        context={'campaign_names':campaign_names,'campaigns':campaigns}
        return render(request, 'campaign.html',context)
